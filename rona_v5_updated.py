...
def get_agent_prompt():
    ...
    system_prompt = (
        ...
        "قواعد مهمة:\n"
        "1. إذا احتاج السؤال لمعلومة حديثة أو غير موجودة محلياً فابدأ بالبحث في الإنترنت تلقائياً\n"
        "2. ركّز أولاً على ما تمتلكه من سياق في قاعدة البيانات المحلية، واذكر صراحةً متى اعتمدت على سجلاتك المحلية\n"
        "3. عند وجود بيانات محلية سابقة صرّح بوضوح أنك تمتلكها بالفعل ثم ادمجها مع نتائج الإنترنت\n"
    ...
    def run_agent_in_thread(self, user_message):
        ...
        try:
            ...
            if self.vector_db is None:
                ...
                try:
                    retrieved_docs = self.vector_db.similarity_search(user_message, k=3)
                    if retrieved_docs:
                        context_header = "📚 تم العثور على سياق من قاعدة البيانات المحلية (أنا أملك بيانات محلية ذات صلة):\n"
